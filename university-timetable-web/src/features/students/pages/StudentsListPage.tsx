...
const CONFLICT_MESSAGE =
    'Студент уже был изменён другим пользователем. Обновите данные и повторите попытку.';

const PRECONDITION_MESSAGE =
    'Не удалось определить версию студента. Обновите страницу и повторите попытку.';
...
export const StudentsListPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (formMode === 'edit' && editingId) {
                ...
                const updated = await studentsApi.update(editingId, body, editingVersion);
    ...
    const handleArchive = async (student: StudentResponse) => {
        if (!isAdmin) return;

        try {
            const updated = await studentsApi.archive(student.id, student.version);
            setItems(prev => prev.map(s => (s.id === updated.id ? updated : s)));
        } catch (e: any) {
            console.error(e);
    ...
    const handleActivate = async (student: StudentResponse) => {
        if (!isAdmin) return;

        try {
            const updated = await studentsApi.activate(student.id, student.version);
            setItems(prev => prev.map(s => (s.id === updated.id ? updated : s)));
        } catch (e: any) {
            console.error(e);
